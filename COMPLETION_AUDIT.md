...
| Enlace privado de informe | `report_token`, `/r/<lead_id>/<token>`, `render_public_report_html()`, `copyReportLink()`, `test_private_report_link.py` | Al generar informe, el backend crea URL privada no indexable con token opaco; la página pública permite copiarla; prueba local real abrió `/r/...`, confirmó `X-Robots-Tag: noindex, nofollow`, contenido de informe y 404 con token incorrecto, sin exponer email ni conversación; release check ejecuta una prueba dedicada sin IA | Hecho base |
