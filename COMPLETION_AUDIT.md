...
| Consentimiento al email-gate | `finalConsent` en `Agente_Real_CRM.html`, `/api/email` en `app_server.py` | El informe exige consentimiento explícito antes de guardar email; CRM guarda `facts.consent`; `test_beta_smoke.py` valida rechazo sin consentimiento y guardado con consentimiento | Hecho base |
