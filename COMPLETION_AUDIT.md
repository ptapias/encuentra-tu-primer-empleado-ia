...
| Gate de posicionamiento y validación local | `release_check.py`, `VALIDACION_LOCAL.md`, `Agente_Real_CRM.html` | `release_check.py` valida que la guía local preserve `./run_local_beta.sh`, el gancho `¿Dónde se te escapa tiempo, dinero o clientes?`, el enfoque consultivo, `Empezar diagnóstico`, `Discovery en vivo`, ausencia de 5 preguntas fijas, `Resumen de acción` y campos CRM `first_opportunity`/`first_step`; también bloquea instrucciones antiguas como `Analizar mi negocio` o arranques manuales obsoletos | Hecho base |
...
- Última verificación de posicionamiento: en `93b4f86`, `release_check.py` incluye `local_validation_doc` y pasa contra una instancia fresca `PORT=8796 AI_PROVIDER=fallback ALLOW_AI_FALLBACK=true ./run_local_beta.sh`; `test_beta_smoke.py --base http://127.0.0.1:8796` y `release_check.py --env /tmp/primer-empleado-valid.env --base http://127.0.0.1:8796` devuelven OK con `/healthz.version=93b4f86`.
