...
| Lista blanca de estáticos públicos | `PUBLIC_STATIC_FILES`, `ADMIN_STATIC_FILES`, `allowed_static_path()` en `app_server.py`, `test_beta_smoke.py` | Solo sirve página pública, privacidad beta y dashboard con auth; bloquea docs, prototipos, scripts, backups y datos internos | Hecho base |
