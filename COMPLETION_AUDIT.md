...
| Actualización VPS segura | `deploy/update_vps.sh`, `release_check.py`, `DEPLOYMENT_VPS.md` | Script hace backup antes de `git pull --ff-only`, usa `git -c safe.directory` para evitar bloqueo por ownership con `sudo`, conserva `.env`/CRM/backups, ejecuta preflight, reinicia systemd y corre smoke local; si hay dominio llama a `verify_vps.sh` | Hecho base |
