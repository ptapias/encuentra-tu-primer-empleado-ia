...
| Preparación de producción | `PRODUCTION_READINESS.md`, `VPS_INPUTS.md`, `validate_vps_inputs.py`, `test_vps_inputs_validator.py` | Lista datos necesarios, variables `.env`, gate final, prueba manual obligatoria, criterios de no apertura y ficha corta verificable para recopilar dominio/SSH/CRM/privacidad/proveedor IA antes de tocar el VPS | Hecho base |
...
| Faltan datos de lanzamiento | Sin dominio, acceso SSH, contraseña CRM y datos legales no se puede completar `--public-beta` | Rellenar `VPS_INPUTS.md`, seguir `VPS_LAUNCH_PACKET.md` y generar privacidad con `render_privacy.py` |
