...
| Enlace privado de informe | `report_token`, `/r/<lead_id>/<token>`, `render_public_report_html()`, `copyReportLink()` | Al generar informe, el backend crea URL privada no indexable con token opaco; la página pública permite copiarla; prueba local real abrió `/r/...`, confirmó `X-Robots-Tag: noindex, nofollow`, contenido de informe y 404 con token incorrecto, sin exponer email ni conversación | Hecho base |
