...
| Systemd coherente con configuración VPS | `deploy/install_vps.sh`, `deploy/primer-empleado-ia.service`, `deploy/primer-empleado-ia-backup.service`, `release_check.py` | El instalador renderiza las unidades systemd con `APP_DIR`, `APP_USER` y `APP_GROUP` reales, evitando que overrides de VPS rompan `WorkingDirectory`, `ExecStart`, `ReadWritePaths` o usuario del servicio | Hecho base |
| Actualización VPS segura | `deploy/update_vps.sh`, `release_check.py`, `DEPLOYMENT_VPS.md` | Script exige worktree limpio, hace backup antes de `git pull --ff-only`, usa `git -c safe.directory` para evitar bloqueo por ownership con `sudo`, conserva `.env`/CRM/backups, ejecuta preflight, reinicia systemd y corre smoke local; si falla tras cambiar de commit intenta rollback al commit anterior; si hay dominio llama a `verify_vps.sh` | Hecho base |
