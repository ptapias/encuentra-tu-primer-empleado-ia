...
| Enlace privado de informe | `report_token`, `/r/<lead_id>/<token>`, `render_public_report_html()`, `copyReportLink()`, `CRM_Dashboard.html`, CSV, `test_private_report_link.py` | Al generar informe, el backend crea URL privada no indexable con token opaco; la página pública permite copiarla; el CRM y el CSV la exponen para operación interna; prueba local real abrió `/r/...`, confirmó `X-Robots-Tag: noindex, nofollow`, contenido de informe y 404 con token incorrecto, sin exponer email ni conversación; release check ejecuta una prueba dedicada sin IA | Hecho base |
...
| CRM con métricas de beta | `/api/metrics`, `CRM_Dashboard.html` | Devuelve leads, inicio, discovery útil, listos para informe, conversión desde listo a email-gate, conversión desde email a informe, feedback, CTA, utilidad, latencia y salud operativa | Hecho |
| CRM con discovery viva antes/después del informe | `CRM_Dashboard.html`, `facts._discovery`, `/api/export.csv` | El detalle del CRM muestra foco, claridad, señales, huecos y procesos candidatos; el CSV exporta `discovery_focus`, `discovery_confidence`, `discovery_ready`, `candidate_processes`, `open_gaps` y `live_insights`; smoke test valida la presencia | Hecho base |
...
| Gate de posicionamiento y validación local | `release_check.py`, `VALIDACION_LOCAL.md`, `Agente_Real_CRM.html`, `test_beta_smoke.py`, `test_public_ui_flow.py` | `release_check.py` valida que la guía local preserve `./run_local_beta.sh`, el gancho `¿Dónde se te escapa tiempo, dinero o clientes?`, `Discovery gratuito · 7-10 min`, el enfoque consultivo, `Empezar diagnóstico`, `Discovery en vivo`, ausencia de 5 preguntas fijas, `Resumen de acción` y campos CRM `first_opportunity`/`first_step`; `test_beta_smoke.py` protege el lateral "Discovery gratuito", "mini sesión consultiva", "Empieza con una escena real", "No hace falta que lo traigas ordenado" y "mapa de procesos, señales y prioridades"; también bloquea instrucciones antiguas como `Analizar mi negocio` o arranques manuales obsoletos | Hecho base |
...
| Plan de beta alineado con email al final | `BETA_TEST_PLAN.md`, `release_check.py` | El plan mide `Inicio -> discovery útil`, `Discovery útil -> ready_for_report`, `ready_for_report -> email-gate aceptado` e `Email-gate aceptado -> informe`; evita volver a `Inicio -> email` y pide revisar escena concreta, repreguntas adaptadas, fuga principal y primer paso recomendado | Hecho base |
...
| Experiencia visual comparable a startup YC | `Agente_Real_CRM.html` con hero fuerte, layout, progreso, tarjetas de proceso, preview de discovery y sidebar consultivo | Revisión visual local en escritorio y móvil: hero claro, gancho `¿Dónde se te escapa tiempo, dinero o clientes?`, lateral alineado con "Discovery gratuito" y "Empieza con una escena real", estado inicial no técnico, compositor oculto hasta empezar, botón inicial no duplicado tras iniciar; `test_public_ui_flow.py --screenshots` genera capturas desktop/móvil. Estándar "YC-level" sigue necesitando test con usuarios | Parcial |
...
- Revisión visual con Chrome/Playwright: hero mantiene el gancho "¿Dónde se te escapa tiempo, dinero o clientes?", el pill móvil se acorta a "Discovery gratuito · 7-10 min", el lateral refuerza "Discovery gratuito", "mini sesión consultiva" y "Empieza con una escena real", informe muestra matriz de decisión y flujo práctico, sin términos internos como JSON/fallback/descargar. En móvil, el compositor queda oculto hasta empezar y aparece activo al iniciar la sesión.
...
- Verificación de enlace privado: en `0ecdda7`, el CSV exporta `public_report_url`; `test_beta_smoke.py --base http://127.0.0.1:8799` y `release_check.py --env /tmp/primer-empleado-valid.env` devuelven OK.
- Última verificación de posicionamiento: en `ee00f8a`, `release_check.py --env /tmp/primer-empleado-valid.env`, `test_beta_smoke.py --base http://127.0.0.1:8802` y `test_public_ui_flow.py --base http://127.0.0.1:8802 --screenshots output/playwright/latest-ui-3` devuelven OK tras alinear hero, lateral y microcopy alrededor de discovery consultiva, escena real y fuga de tiempo/dinero/clientes.
