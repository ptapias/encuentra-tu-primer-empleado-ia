...
- Estado de preparación actual: `python3 beta_readiness_status.py` devuelve `blocked_on_launch_inputs` y recomienda `python3 generate_vps_inputs.py` como primera acción; desde `a63c176` también muestra la ruta no interactiva con `VPS_ANSWERS.local.json`.
- Estado listo futuro: `test_beta_readiness_status.py` valida que, cuando existen inputs/env/privacidad/prueba manual, el siguiente paso sea `deploy/verify_vps.sh` con `PUBLIC_BETA=true`, `MANUAL_PRODUCTION_TESTED=true`, `CRM_REVIEWED=true` y micro probado u opcional explícito.
